...
def test_a_chat_that_rose_had_activity():
    assert "Alex Kumar" in chat_watch.moved_up(
        ["Dana", "Alex Kumar", "Team"], ["Alex Kumar", "Dana", "Team"])
...
def test_the_top_chat_is_always_checked():
    ...
    assert chat_watch.moved_up(["Alex", "Dana"], ["Alex", "Dana"]) == ["Alex"]


def test_a_brand_new_conversation_counts():
    assert "Newperson" in chat_watch.moved_up(["Dana"], ["Newperson", "Dana"])
...
def test_a_chat_that_sank_is_not_reopened():
    ...
    assert "Dana" not in chat_watch.moved_up(["Dana", "Alex"], ["Alex", "Dana"])


# --- Asta's high-water mark, not Teams' read state ----------------------------

def _msgs(*keys):
    return [{"key": k, "text": f"message {k}", "sender": "Alex Kumar"} for k in keys]


def test_only_messages_after_the_mark_are_new():
    store.kv_set(chat_watch._seen_key("Alex"), "m2")
    assert [m["key"] for m in chat_watch.unseen("Alex", _msgs("m1", "m2", "m3", "m4"))] \
        == ["m3", "m4"]


def test_the_same_poll_twice_yields_nothing_the_second_time():
    rows = _msgs("m1", "m2")
    store.kv_set(chat_watch._seen_key("Alex"), "")
    chat_watch.unseen("Alex", rows)
    chat_watch.remember("Alex", rows)
    assert chat_watch.unseen("Alex", rows) == []
...
def test_a_mark_that_scrolled_out_of_view_replays_rather_than_loses():
    ...
    store.kv_set(chat_watch._seen_key("Alex"), "long-gone")
    assert len(chat_watch.unseen("Alex", _msgs("m8", "m9"))) == 2
...
def wired(monkeypatch):
    ...
    bridge = _Bridge({"Alex Kumar": [
        {"key": "k1", "sender": "Alex Kumar",
    ...
    store.kv_set(chat_watch._seen_key("Alex Kumar"), "")
    return bridge
...
def test_an_untagged_one_to_one_message_is_handled(wired):
    ...
    assert handled[0]["who"] == "Alex Kumar"
...
def test_his_own_messages_are_not_things_he_was_asked(monkeypatch):
    ...
    assert not chat_watch.is_from_him("Alex Kumar")
...
def test_his_own_self_chat_is_not_a_conversation():
    ...
    assert not chat_watch.is_furniture("Alex Kumar")
...
def test_a_group_message_to_nobody_is_not_his():
    ...
    assert not chat_watch.addressed_to_him("Prod Support", "Dana", "deploy done")


def test_a_tag_makes_the_group_his():
    assert chat_watch.addressed_to_him("Prod Support", "Dana",
...
# the previous order. The stored rail had "Blake" and "Stone Blake Rivers"
...
def test_the_real_chat_list_is_recognised():
    assert chat_watch.looks_like_the_chat_list(
        ["Copilot", "Mentions", "Discover", "Drafts", "Saved", "Alex Kumar"])
...
def test_search_results_are_not_mistaken_for_the_rail():
    ...
    assert not chat_watch.looks_like_the_chat_list(
        ["Blake", "Stone Blake Rivers", "BEP_Telikos : Defect Triage"])
...
def test_a_contaminated_rail_is_never_remembered(monkeypatch):
    ...
    store.kv_set(chat_watch._RAIL_KEY, '["Alex Kumar", "Dana Frost"]')
    ...
    asyncio.run(chat_watch.sweep())
    import json
    assert json.loads(store.kv_get(chat_watch._RAIL_KEY)) == ["Alex Kumar", "Dana Frost"]
...
def test_a_partly_working_sweep_counts_as_healthy(monkeypatch):
    ...
    async def _mixed(chat, advance=True):
        ...
        return [{"key": "k", "sender": "Alex", "text": "prod is stuck"}]
...
# we join here now?" and "Hi Kendall just wanted to check what we have concluded" —
...
def test_group_chatter_between_other_people_is_not_forwarded(monkeypatch):
    ...
    sent = _sweep_with(monkeypatch, "BEP_Telikos : Defect Triage", "Noor Kumar",
                       "Hi Kendall just wanted to check what we have concluded")
...
def test_a_one_to_one_is_always_forwarded(monkeypatch):
    ...
    sent = _sweep_with(monkeypatch, "Glen Hart", "Glen Hart",
...
def test_unforwarded_group_traffic_is_still_recorded(monkeypatch):
    ...
    _sweep_with(monkeypatch, "BEP_Telikos : Defect Triage", "Noor Kumar",
...
def test_a_one_to_one_names_the_person_once():
    line = chat_watch.render("Stone Blake Rivers", "Stone Blake Rivers",
                             "hello", 1)
    assert line.count("Stone Blake Rivers") == 1


def test_a_group_message_says_which_group():
    """Where it was said is the part he cannot infer from the sender."""
    line = chat_watch.render("BEP_Telikos : Defect Triage", "Noor Kumar", "hi", 1)
    assert "Noor Kumar" in line and "BEP_Telikos" in line


#: A real reply, captured verbatim from his store. Sender was Reese Wells;
#: the first three lines are Oakley Kumar's message being replied TO.
REAL_REPLY = (
    "Oakley Kumar\n20/07/2026 11:14\nReese - What is IP. For NAM, in the "
...
def test_a_reply_shows_what_the_sender_typed_not_what_they_quoted():
    """The misattribution he caught: "msg was mine but u shoing blake".
...
@pytest.mark.parametrize("url,expected", [
    ("https://github.com/example-dev/incident-copilot", "GitHub: example-dev/incident-copilot"),
...
def test_a_message_that_is_only_a_link_says_what_was_shared():
    out = chat_watch.summarise("https://github.com/example-dev/incident-copilot")
...
def test_the_rest_of_the_room_does_not_come_with_it():
    ...
    chat_watch.addressed_to_him(CHAT, "Alex Kumar", "Arunkumar, could you look")
    for who, text in [("Peyton R", "2) I found this issue for two Service Plan numbers"),
                      ("Hayden Kumar Thorne", "22nd September ko release hai"),
                      ("Hayden Kumar Thorne", "Also inform telemetry now")]:
        assert not chat_watch.addressed_to_him(CHAT, who, text), f"{who}: {text}"


def test_a_message_aimed_at_a_third_person_is_theirs():
    """"Alex Kumar what do you say" is a question for Alex."""
    chat_watch.addressed_to_him(CHAT, "Alex Kumar", "Arunkumar, could you look")
    assert not chat_watch.addressed_to_him(CHAT, "Hayden Kumar Thorne",
                                           "Alex Kumar what do you say")
...
def test_naming_himself_is_not_naming_someone_else():
    assert not chat_watch.names_someone_else("Alex Kumar here, any update", "Alex Kumar")
...
def test_an_ordinary_sentence_is_not_read_as_addressing_anyone():
    ...
    assert not chat_watch.names_someone_else("Activity getting missed in prod", "Peyton R")
...
def test_a_thread_he_has_replied_in_is_not_investigated_again(monkeypatch):
    ...
    monkeypatch.setattr(store, "teams_messages",
                        lambda chat="", limit=200, **k: [
                            {"sender": "Alex Kumar", "text": "look into these", "sent_at": 100},
...
def test_an_unanswered_ask_is_still_picked_up(monkeypatch):
    monkeypatch.setattr(store, "teams_messages",
                        lambda chat="", limit=200, **k: [
                            {"sender": "Alex Kumar", "text": "look into these", "sent_at": 300}])
