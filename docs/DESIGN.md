...
*The long version: why each part is built the way it is, and what went wrong
before it was. The [README](../README.md) is the short one.*
...
| *"make me an excel of my open tasks and send it"* | A brain decides the rows; **code** writes the xlsx (or csv, docx, pptx, pdf), re-opens it to check the rows arrived, and sends it to your phone as a document |
| *"remind me tomorrow morning to chase the review — put it in Reminders"* | Asks Reminders through its own AppleScript door, then **reads the list back** before saying it is done |
| *"keep an eye on PR 17, warn me if it isn't merged by 5"* | A checkpointed promise: it watches, warns **before** the deadline, and waits for your "chase them" or "leave it" — across restarts |
| *"don't include PR reviews in my standup — remember this always"* | Proposed as a standing rule; on your yes it is enforced by code and written into your `guardrails.md` |
| *"status"*, *"what have you changed"*, *"roll back 1"* | Answered from state in well under a second — no brain, no tokens |
...
```bash
.venv/bin/python -m pytest -q                       # ~3,000 tests
.venv/bin/python -m app.workworld run --k 4         # the scenario bench, pass^4
```
...
### Checkpointed work (LangGraph)

With `ASTA_GRAPH=1` a code task is a **checkpointed thread** (`app/graph/`):
plan → announce → gate ⏸ → implement → verify, where ⏸ is a LangGraph
`interrupt()`. The thread *is* its checkpoint, so a restart, a usage limit or a
closed lid resumes the job where it stopped instead of starting it again. Two rules
make that safe: a gate node does nothing but wait (a resumed node re-runs from its
start), and your answer at a gate is **written down before it is applied** — an
approval that arrives a moment before a restart is otherwise lost twice over.

Every decision is taken from the leg's **typed outcome** (`report_outcome`, or a
fenced `outcome` block), never by reading prose for "done".

`ASTA_GRAPHS=1` adds three more threads on the same engine (`app/graph/engine.py`):

- **Investigate** — findings are state, so a limit halfway through keeps what was
  established; one look waits for its analysis and a re-run reuses it rather than
  starting a second; it asks you one question rather than guessing, and stops when
  the looks run out.
- **Follow through** — a promise with a deadline warns *before* it lands and parks
  for your "chase them" / "leave it"; the front desk records your word and the next
  tick applies it. Nudges to the person are staged, never sent.
- **Draft and send** — draft → stage → gate ⏸ → send → confirm. A "no" with words
  rewrites the draft; a bare "no" drops it; a send that cannot be confirmed is
  reported as not sent.

The graphs own the *sequence*; `followup`, `responder`, `consent` and `policy` keep
the knowledge, so no decision has a second copy.
...
## The front desk — answers from state, rules for everything else

`app/frontdesk.py` sits in front of every brain (`ASTA_FRONTDESK=1`).

**Questions the tables already answer never reach a model**: status, "status of
117", what is pending or running, open PRs, your rules, your permissions, the
digest, "what have you changed". Answered in well under a second.

**Binding a message to a running job is decided by rules, never by a brain's
guess.** A named job gets it; an unnamed message that no rule can place is answered
on its own, next — never quietly stapled onto whatever happens to be running.

**Standing instructions become rules the code enforces** (`app/instructions.py`,
`app/policy.py`). "Don't investigate incidents going forward", "my default
workspace is booking", "never push unless I ask" are compiled into a typed rule,
offered back once with a one-tap yes, and on your yes checked at the point of the
act (`policy.check`), appended to `guardrails.md`, and guarded by a replay test. A
rule you gave is listed by "my rules" and removed by "drop rule 2".

**Permissions are earned, not assumed** (`app/authority.py`). After ten approvals
of the same kind of act Asta may offer to stop asking — capped per day, revocable
with "revoke 3", and never for anything irreversible.
...
### Picking the model by how hard the work is

With `ASTA_ROUTING=1` (`app/routing.py`) each leg is given a tier — T1 for
trivial, T2 for ordinary, T3 for hard — and each brain maps tiers to its own
models. Plans always run at T2; implementation runs at the tier the approved plan
declared. "use claude", "cheap", or a trailing ", max" override it for one ask.
...
`app/workworld/` is a scenario bench over a **sandboxed world**: a temp database,
scripted brains, a virtual clock, and every outward door — Teams, WhatsApp,
documents, voice, Jira writes, your apps, the screen — replaced by a recorder. A
scenario is YAML (`tests/workworld/*.yaml`): a setup, steps in your words, and
checks on what reached your phone, what was sent, what was written.

```bash
.venv/bin/python -m app.workworld run --k 4 --engine graph    # pass^4
.venv/bin/python -m app.workworld run --set hands             # one set
.venv/bin/python -m app.workworld day                         # a simulated day
```

`pass^k` counts a scenario only if **every** one of `k` runs passes. A **day in
your life** replays ~150 events and reports pushes, things missed and noise
interruptions. The **constitution** set holds the rules that must never regress.
`ASTA_BENCH_NIGHTLY=1` runs a live tier on real brains between 01:00 and 05:00 on
quota that would otherwise expire, and skips itself if you used a brain within the
hour.

The rule for changes: **a newly reported problem gets a scenario before it gets a
fix**, so the bench fails for the right reason first.

**The sandbox owns every door, and the suite does too.** Both were learned the
hard way: the bench once wrote files into the real `~/Asta files` and delivered
documents to a real phone, and five tests pushed a real WhatsApp message on every
run of the suite. `tests/conftest.py` points the database, the WhatsApp bridge,
Telegram, the audio switcher, the voice server and the AppleScript interpreters at
nothing, for every test.

## When Asta changes itself

`ASTA_EVOLVE=1` (`app/evolve.py`, `app/settings.py`): observe → diagnose → propose
→ **prove** → promote → roll back.

- It may move a **short list of numbers, each inside bounds**. Your own limits —
  how many interruptions a day you will take — are not on that list.
- Nothing is kept unless it **beats the version running now** on the same bench
  and the same simulated day, with nothing newly missed.
- The constitution scenarios are **checked, not trusted**: a candidate that breaks
  one is refused however much else it improves.
- A promoted change runs under a 24-hour canary. "what have you changed" lists
  every change with its numbers; "roll back 1" undoes one.
- A code fix is only ever a **proposal**: it writes the failing scenario first and
  stops at your plan gate like any other task.

It runs in the night window when you have not used a brain for an hour.
...
## Hands — files, apps, the screen, a voice

In order of preference. The more reliable door is always taken when it exists.

**Files** (`app/files.py`) — "give me this in Excel" produces a real file. A brain
decides the rows; code writes xlsx, csv, md, docx, pptx or pdf, then **re-opens what
it wrote** and checks the header, the rows and the text arrived (a PDF is read back
by a different library from the one that wrote it). A file that fails the check is
never sent. It arrives on your phone as a WhatsApp document, or you are told where
it is on the Mac. Files live in `ASTA_FILES_DIR`.

**Your apps, through their own doors** (`app/apps.py`, `ASTA_APPS=1`) — Reminders
(with a due time), Calendar, Notes, a draft in **Outlook** (or Apple Mail), showing
a file in Finder, and your own Shortcuts. Three rules:

- **A named recipe, never a free-form script.** A brain picks a recipe and fills in
  arguments; anything that can write AppleScript can empty your calendar.
- **Arguments are passed, never pasted** into the script — a title full of quotes
  and em dashes is data, not code.
- **Every write is read back.** The act is what the app shows afterwards, not the
  interpreter's exit code. Mail can draft and cannot send; sending keeps its own gate.

**The screen and the mouse** (`app/screen.py`, `ASTA_SCREEN=1`) — the last resort,
for an app with no other way in. Elements by **name**, never coordinates. Every step
states what must be true afterwards and it is checked before the next step runs; a
click whose expectation does not come true stops the sequence. Text the keyboard
cannot type (an em dash) is pasted, and your clipboard is given back. **Refused
outright for any app that has a real door**, and a path that worked is kept to replay.

**A voice** (`app/voice.py`) — "tell me out loud" leaves a short voice note on
WhatsApp in Asta's voice, for what you would rather hear while walking; never for
an outcome or a file you will want to search later. Opus needs `ffmpeg`; without it
the note goes as an audio clip, and says so.

**macOS asks once per app.** Apps need *Privacy & Security → Automation* for the
Python that runs Asta; the screen also needs *Accessibility*. Until you allow them,
Asta says exactly which switch is missing — Automation and Accessibility are
different pages, and naming the wrong one leaves the real switch off.
...
**Never send `--tools` with an MCP config to the Claude CLI.** `--tools` does not
narrow the built-in tools; it replaces the whole tool set, MCP included — measured:
zero `mcp__asta` tools with it, nineteen without. Heavy built-ins are trimmed with
`--disallowed-tools` instead. Every Claude session also runs with
`CLAUDE_CODE_DISABLE_AUTO_MEMORY=1` — Asta's memory is Asta's — and with stdin
closed, because the CLI appends piped stdin to the prompt.
...
**A rejected token is not an empty backlog.** Jira does not refuse an anonymous
search — it answers `200` with no issues. So an expired or revoked token made every
Jira read silently empty: the brief, the standup, "what's on my plate". An empty
answer now checks who Jira thinks Asta is (`/myself`), raises `JiraAuthError` when
the token is rejected, and health reports it; the standup says it cannot read Jira
instead of reporting a quiet sprint.
...
- **Morning brief** (`BRIEF_TIME=08:30`, weekdays) — finished work, things waiting
  ...
  tokens.
- **Standup draft** (`STANDUP_TIME=09:15`, weekdays) — **only the Jira tickets
  assigned to you in the current sprint**: in progress, to do, or finished in the
  last day. No PR reviews, no file or document work, no commits, nothing Asta did
  for you — a standup is about your tickets. Tickets finished long ago in a sprint
  that is still technically open are left out. Phrased by whichever brain still has
  quota, never inside a code block.
...
**A daily budget, a digest, and learning from what you ignore.** `ASTA_PUSH_BUDGET`
(20) is how many interruptions a day you will take, counted at the delivery door.
Past it, ordinary news goes to the midday and evening **digest** — breakage still
buzzes. With `ASTA_ATTENTION_LEARN=1`, a bulk sender you consistently ignore is
moved to the digest, with one line saying so and "push <name>" to undo it. Only
bulk senders are ever demoted — never a colleague, and never Asta's own voice.

**A mail about alerts is not an alert.** The hold window is for alerting systems. A
talk, a webinar, a workshop, a newsletter or an invitation is not breakage however
alarming its title ("… & Alert Enrichment — please join us"); an unmistakable
alerting subject (`[FIRING]`, `P1:`, `SEV1:`) still is.
...
Everything new is **off by default** and documented in `.env.example`. The ones
worth knowing:

| Flag | What it turns on |
|---|---|
| `ASTA_GRAPH` | code tasks as checkpointed LangGraph threads |
| `ASTA_GRAPHS` | the investigate / follow-through / draft-and-send threads |
| `ASTA_FRONTDESK` | answers from state; rules-only binding; standing rules |
| `ASTA_ROUTING` | model tier by how hard the work is |
| `ASTA_PUSH_BUDGET` | interruptions per day before news goes to the digest |
| `ASTA_ATTENTION_LEARN` | demote bulk senders you consistently ignore |
| `ASTA_EVOLVE` | self-evolution inside bounds, proved before it is kept |
| `ASTA_APPS` | Reminders, Calendar, Notes, Outlook, Finder, Shortcuts |
| `ASTA_SCREEN` | the screen-and-mouse fallback |
| `ASTA_BENCH_NIGHTLY` | the live bench tier, overnight, on spare quota |
| `ASTA_VOICE_NOTE` | a voice note when a call goes unanswered — prove it on your own chat first |
...
- The app and screen doors run under **your own** macOS permissions (Automation,
  Accessibility) and are off unless `ASTA_APPS` / `ASTA_SCREEN` are set. Neither the
  suite nor the bench can reach them.
...
app/graph/              checkpointed threads: code, investigate, follow through, draft & send
app/frontdesk.py        answers from state; binds messages to work by rules
app/routing.py          which model, by how hard the work is
app/policy.py           standing rules, checked at the point of the act
app/instructions.py     "from now on …" → a typed rule → guardrails.md
app/authority.py        permissions earned after repeated approvals, revocable
app/budget.py           the daily interruption budget
app/digest.py           what waits for midday and evening instead of buzzing
app/settings.py         the bounded knobs self-evolution may move
app/evolve.py           observe → diagnose → propose → prove → promote → roll back
app/files.py            xlsx/csv/md/docx/pptx/pdf written by code, read back, delivered
app/apps.py             Reminders, Calendar, Notes, Outlook, Finder — named recipes, read back
app/screen.py           the last resort: elements by name, every step checked
app/briefing.py         the morning brief and the sprint-only standup
app/workworld/          the scenario bench: sandboxed world, pass^k, a simulated day
...
tests/                  ~3,000 tests (conftest isolates the DB and every outward door)
tests/workworld/        the bench scenarios, one YAML file per set
```
