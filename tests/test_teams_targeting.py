...
first person-shaped row that matched any token. "Kumar" quietly opened Alex
Kumar while Glen, Anil and Hayden Kumar sat in the same list.
...
def test_one_match_is_simply_used():
    picked = teams_bridge._one_of([_opt(6, "Person  Alex Kumar", "Alex Kumar")],
                                  "Alex", "people")
    assert picked["i"] == 6
...
def test_a_surname_matching_several_people_is_refused_not_guessed():
    ...
    people = [_opt(1, "Person  Alex Kumar", "Alex Kumar\n\nSOFTWARE ENGINEER"),
              _opt(2, "Person  Glen Kumar", "Glen Kumar\n\nLEAD"),
              _opt(3, "Person  Hayden Kumar Thorne", "Hayden Kumar Thorne\n\nQA")]
    with pytest.raises(RuntimeError) as exc:
        teams_bridge._one_of(people, "Kumar", "people")
    msg = str(exc.value)
    assert "matches 3 people" in msg
    assert "Glen Kumar" in msg and "Alex Kumar" in msg
    assert "which one he means" in msg


def test_an_exact_name_wins_over_a_longer_one_that_also_matches():
    """Otherwise having a "Alex Kumar Singh" in the directory would make
    "Alex Kumar" permanently unsendable."""
    people = [_opt(1, "Person  Alex Kumar", "Alex Kumar"),
              _opt(2, "Person  Alex Kumar Singh", "Alex Kumar Singh")]
    assert teams_bridge._one_of(people, "Alex Kumar", "people")["i"] == 1
...
def test_the_same_person_listed_twice_is_one_candidate():
    ...
    rows = [_opt(1, "Person  Alex Kumar", "Alex Kumar\n\nSOFTWARE ENGINEER"),
            _opt(4, "Person  Alex Kumar", "Alex Kumar\n\n(ALEX.KUMAR)")]
...
def test_the_display_name_ignores_the_role_line_teams_appends():
    assert teams_bridge._display_name(
        _opt(1, "", "Alex Kumar\n\n(ALEX.KUMAR) SOFTWARE ENGINEER")) == "Alex Kumar"
...
def test_someone_he_is_already_talking_to_wins_over_the_directory():
    """Two Caseys are top hits. One is in his rail. That settles it."""
    rows = [_p(1, "Casey Jesse", top=True), _p(2, "Casey Shaikh", top=True)]
    picked = teams_bridge._one_of(rows, "Casey", "people", {"casey jesse"})
    assert teams_bridge._display_name(picked) == "Casey Jesse"
...
def test_a_shared_surname_resolves_to_the_one_he_has_a_chat_with():
    ...
    rows = [_p(1, "Alex Kumar", top=True), _p(2, "Hayden Kumar Thorne", top=True),
            _p(3, "Sasha Kumar"), _p(4, "Marlowe Kumar Singh")]
    picked = teams_bridge._one_of(rows, "Kumar", "people", {"alex kumar"})
    assert teams_bridge._display_name(picked) == "Alex Kumar"
...
def test_two_people_he_talks_to_is_still_a_refusal_and_says_why():
    ...
    rows = [_p(1, "Casey Jesse", top=True), _p(2, "Casey Shaikh", top=True)]
    with pytest.raises(RuntimeError) as exc:
        teams_bridge._one_of(rows, "Casey", "people", {"casey jesse", "casey shaikh"})
...
def test_a_stranger_is_reachable_by_full_name():
    ...
    rows = [_p(1, "Harini S", top=True, chat=True), _p(2, "Frankie Reddy")]
    picked = teams_bridge._one_of(rows, "Frankie Reddy", "people", {"hale frankie"})
    assert teams_bridge._display_name(picked) == "Frankie Reddy"


def test_the_exact_name_still_outranks_the_rail():
    """Otherwise having chatted with Alex Kumar would make "Alex Kumar Balaji"
    permanently unreachable even when spelled out in full."""
    rows = [_p(1, "Alex Kumar", top=True), _p(2, "Alex Kumar Balaji")]
    picked = teams_bridge._one_of(rows, "Alex Kumar Balaji", "people", {"alex kumar"})
    assert teams_bridge._display_name(picked) == "Alex Kumar Balaji"
...
def test_an_empty_rail_falls_back_to_the_directory_ranking():
    ...
    rows = [_p(1, "Alex Kumar", top=True), _p(2, "Alex Kumar Balaji")]
    assert teams_bridge._display_name(
        teams_bridge._one_of(rows, "Alex", "people", set())) == "Alex Kumar"
...
def test_the_rail_drops_navigation_furniture():
    rows = ["Copilot", "Mentions", "Drafts", "Saved", "Alex Kumar", "prod issue - triaging"]
    got = asyncio.run(teams_bridge.recent_chats(_RailPage(rows)))
    assert "alex kumar" in got and "prod issue - triaging" in got
    assert "copilot" not in got and "drafts" not in got


def test_the_rail_is_remembered_because_teams_only_renders_part_of_it():
    """The bug this exists for: the rail is virtualised, so one poll saw Casey
    Jesse and the next did not — the same question answered two ways with
    nothing changed but scroll position."""
    asyncio.run(teams_bridge.recent_chats(_RailPage(["Casey Jesse", "Alex Kumar"])))
    later = asyncio.run(teams_bridge.recent_chats(_RailPage(["Alex Kumar"])))
    assert "casey jesse" in later, "a name seen once must not vanish on the next poll"


def test_a_rail_that_cannot_be_read_still_returns_what_was_remembered():
    asyncio.run(teams_bridge.recent_chats(_RailPage(["Alex Kumar"])))
    assert "alex kumar" in asyncio.run(teams_bridge.recent_chats(_RailPage([], boom=True)))
...
def test_a_thread_title_matches_on_any_token_he_used():
    assert teams_bridge._title_matches("Alex Kumar", "alex")
...
def test_a_different_persons_thread_never_counts_as_a_match():
    assert not teams_bridge._title_matches("Casey Jesse", "ellis")
...
def test_an_approved_teams_draft_becomes_a_recorded_call():
    staged = {"channel": "teams", "to": "Alex Kumar",
...
def test_the_text_is_carried_verbatim_not_summarised():
    ...
    op = main._mechanical_send({"channel": "teams", "to": "Frankie", "what": exact})
...
def test_the_send_op_is_registered_and_runs_the_real_bridge(monkeypatch):
    ...
    line = asyncio.run(ops.run({"name": "teams_send",
                                "args": {"to": "Alex", "text": "ping", "to_group": False}}))
    assert sent == {"chat": "Alex", "text": "ping", "allow_group": False}
    assert "Sent to Alex Kumar" in line
...
def test_a_failed_send_is_not_reported_as_success(monkeypatch):
    ...
    async def refuses(chat, text, allow_group=False):
        raise RuntimeError("opened 'Alex Kumar Singh' instead of 'Alex' — aborted")
...
def test_the_offer_names_the_group_so_he_sees_it_before_saying_yes():
    ...
    assert "GROUP" not in describe({"to": "Alex", "to_group": False})
...
def test_staging_defaults_to_a_person():
    ...
    loop.set_pending_send("c2", "hi", "Alex", "teams")
